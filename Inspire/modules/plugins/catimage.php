<?php
/**
 * Theme catimage function file
 * @package Louie
 * @since Theme version 1.0.0
 */
define('Z_IMAGE_PLACEHOLDER', THEME_DEFAULT_URL);
...
function get_taxonomy_image_url() {
	$posttags = get_the_tags();
	if ($posttags) {
	  	foreach($posttags as $tag) {
	    	$count++;
	    	if (1 == $count) {
	      		$tag_image = $tag->term_id;
	    	}
	  	}
	}
	echo z_taxonomy_image_url($tag_image,true,true);
}

function get_cats_image_url() {
	$cats = get_the_category();
	if ($cats) {
	  	foreach($cats as $cat) {
	    	$count++;
	    	if (1 == $count) {
	      		$cat_image = $cat->term_id;
	    	}
	  	}
	}
	echo z_taxonomy_image_url($cat_image,true,true);
}
