<?php
$approve = object('site_mail_approve');
$reply = object('site_mail_reply');
$login = object('site_mail_login');
$update = object('site_mail_update');
$delete = object('site_mail_delete');
$newpost = object('site_mail_newpost');
$color = object( 'site_color' );

 //定义界面顶部区域内容,请注意修改您的主题目录
$name = wp_get_theme()->display('Name');
//$email_bg = THEME_URL .'/images/email.jpg';
    ...
    <div class="emailpaged" style="background: #fff;">
        <div class="emailcontent" style="width:100%;max-width:720px;text-align: left;margin: 0 auto;padding-top: 20px;padding-bottom: 80px">
            <div class="emailtitle" style="border-radius: 5px;border:1px solid #eee;overflow: hidden;">
                <h1 style="color:#fff;background: '.$color.';line-height:70px;font-size:24px;font-weight:normal;padding-left:40px;margin:0">
';
...
$email_footer = '
                ...
                <p style="color: #6e6e6e;font-size:13px;line-height:24px;text-align:right;padding:0 32px">邮件发自：<a href="' . get_bloginfo('url') . '" style="color:#51a0e3;text-decoration:none">' . get_option("blogname") . '</a></p>
            </div>
        </div>
    </div>
';
...
function new_from_name($email){
    $wp_from_name = object('site_mail_name');
    return $wp_from_name;
}
function new_from_email($email) {
    $wp_from_email = object('site_mail_address');
