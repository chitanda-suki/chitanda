...
if( ! function_exists( 'cs_admin_enqueue_scripts' ) ) {
  function cs_admin_enqueue_scripts() {
    ...
    // theme updata script
    wp_enqueue_script('cs-update', CS_URI . '/assets/js/cs-update.js', array(), '1.0.0', true);

    wp_localize_script( 'cs-update', 'Update' , array(
        'domain' => substr(home_url(),strpos(home_url(),'/')+2),
    ));
